...
function asigcuradi()
{
	...
	var x=$('input:radio[name=acestado]:checked').val();
	if(x==null)
	{
		alert('debe seleccionar un dato');
		$.ajax({
		url:'../../site_media/html/asignar_cursos/ac_table.php',
		type:'get',
		success: function (data)
		{
			$("#body_admin").html(data);
	...
	else
	{
		...
		//alert(x);
		var y="AsignaCursosCtrl.php?cur="+x+"&pagina=adiciona&"
		 $.ajax({
	url:y,
	type:'get',
	success: function (data){
